...
PGPASSWORD="$GREENPLUM_PASSWORD" psql -v ON_ERROR_STOP=1 --username "$GREENPLUM_USER" --dbname "$GREENPLUM_DATABASE_NAME" <<-EOSQL
    ...
    GRANT CONNECT ON DATABASE ${GREENPLUM_DATABASE_NAME} TO grp_admins;
    CREATE ROLE grp_supports;
    GRANT CONNECT ON DATABASE ${GREENPLUM_DATABASE_NAME} TO grp_supports;
    CREATE ROLE adb_exec_dwh;
    GRANT CONNECT ON DATABASE ${GREENPLUM_DATABASE_NAME} TO adb_exec_dwh;
